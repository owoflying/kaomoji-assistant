...
class UserTriggers(QObject):
    ...
    def match(self, text):
        """返回尾部命中触发词对应的 output；无命中返回 None。

        取最长匹配，避免「k」先命中而「kk」后到的短词误触发。
        """
        ...
        for it in self.items:
            ...
            if trig and trig in text:
                if best is None or len(trig) > len(best[0]):
                    best = (trig, it["output"])
        return best[1] if best else None

    def add(self, trigger, output):
        ...
        for it in self.items:
            ...
            self.items.append({"trigger": trigger, "output": output})
        self._schedule_save()
        self.changed.emit()
