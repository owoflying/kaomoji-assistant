...
class HomePage(QWidget):
    ...
    def _init_ui(self):
        ...
        hello.setFont(QFont("Segoe UI Variable", 18, QFont.Weight.Bold))
        hello.setStyleSheet("color:" + ("#f0f0f0" if self.config.get("theme") == "dark" else "#1f1f1f") + ";")
    ...
    def _stat_card(self, label, value):
        ...
        root.setSpacing(4)
        val = QLabel(value)
        val.setFont(QFont("Segoe UI Variable", 22, QFont.Weight.Bold))
        val.setStyleSheet("color:" + ("#60cdff" if self.config.get("theme") == "dark" else "#0067c0") + ";")
        lb = QLabel(label)
